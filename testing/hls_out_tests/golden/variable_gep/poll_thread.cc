...
{
  hls::stream<bytes<36> > stage0_port0_stream("stage0_port0");
  bytes<36>               stage0_port0_buffer;
  ...
  while (true) {
    ...
    if (stage0_port0_stream.empty()) {
      if (nanotube_channel_try_read(context, 0, &stage0_port0_buffer, 36)) {
        active = true;
...
void nanotube_setup()
{
  ...
  channels[0] = nanotube_channel_create("packets_in", 36, 16);
