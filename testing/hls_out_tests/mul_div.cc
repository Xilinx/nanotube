...
**  \brief  A multiple and divide test for the HLS output pass.
**   \date  2020-08-08
*//*
\**************************************************************************/
...
void logic_0(nanotube_context_t* context, void *arg)
{
  ...
  word.bytes[0] = word.bytes[0] * word.bytes[1];
  word.bytes[2] = word.bytes[2] / word.bytes[3];
  word.bytes[4] = word.bytes[4] % word.bytes[5];
  word.bytes[6] = int8_t(word.bytes[6]) / int8_t(word.bytes[7]);
  word.bytes[8] = int8_t(word.bytes[8]) % int8_t(word.bytes[9]);
