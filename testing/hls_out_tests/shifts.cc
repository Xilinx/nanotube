...
**  \brief  A shift instruction test for the HLS output pass.
**   \date  2020-08-11
*//*
\**************************************************************************/
...
void logic_0(nanotube_context_t* context, void *arg)
{
  ...
  word.bytes[1] ^= (word.bytes[0] << 3);
  word.bytes[3] ^= (word.bytes[2] >> 3);
  word.bytes[5] ^= (int8_t(word.bytes[4]) >> word.bytes[6]);
