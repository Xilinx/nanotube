/*******************************************************/
/*! \file test_array_maps.cpp
** \author Stephan Diestelhorst <stephand@amd.com>
**         Neil Turton <neilt@amd.com>
**  \brief Unit tests for Nanotube array map implementation.
**   \date 2020-03-04
*//******************************************************/
...
/* The capacity of the main map. */
static const uint32_t work_map_capacity = nanotube_array_map_capacity;

/* Golden keys and data for canary / sentinel checking */
static uint8_t sentinel_key[SENTINELS][KEY_WIDTH];
static uint8_t sentinel_data[SENTINELS][64];
static uint8_t test_key[KEY_WIDTH];

static void write_key(uint8_t key[KEY_WIDTH], uint32_t key_id)
{
...
/*!
** Generate the keys to be used in the test.
**
** Generates the sentinel keys and the test key.  All the generated
** keys will be different and in the required range.
*/
static void generate_keys()
{
  uint32_t base_id = uint32_t(rand()) % work_map_capacity;
  write_key(test_key, base_id);

  /* Generate random keys */
  uint32_t min_step = 1;
  uint32_t max_step = (work_map_capacity-1) / SENTINELS;
  assert(max_step >= min_step);

  uint32_t key_id = base_id;
  for( int i = 0; i < SENTINELS; ++i ) {
    uint32_t step = min_step + (rand() % (max_step+1-min_step));
    uint32_t prev_key_id = key_id;
    key_id = (key_id + step) % work_map_capacity;
    assert(key_id != base_id);
    assert(key_id < base_id || prev_key_id >= base_id);
    write_key(sentinel_key[i], key_id);
  }

  uint32_t index = (rand() % (SENTINELS+1));
  if (index < SENTINELS) {
    std::swap(test_key, sentinel_key[index]);
  }
}
...
static void add_sentinel_data(nanotube_context_t* ctx,
  ...
  int len;
  sentinel_map = nanotube_map_create(SENTINEL_MAP, NANOTUBE_MAP_TYPE_ARRAY_LE,
                                     KEY_WIDTH, 64);
...
/*!
** Run the tests which don't cause a fatal error.
**/
static void normal_map_tests(nanotube_context_t *ctx,
                             nanotube_map_id_t A_id)
{
  ...
  /* Simple map write */
  len = nanotube_map_write_masked(ctx, A_id, test_key, sizeof(test_key), data,
                                  all_enabled, 0, 64);
  ...
  uint8_t data2[64];
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key), data2, 0, 64);
  check_sentinel_data(ctx, A_id);
  ...
  size_t pos = 16;
  data[pos]  = 0xab;
  len = nanotube_map_write(ctx, A_id, test_key, sizeof(test_key), data + pos, pos, 1);
  check_sentinel_data(ctx, A_id);
  printf("Wrote %zu bytes at pos %zu value %02x.\n", len, pos, data[pos]);
  assert_eq(len, 1);

  pos = 10;
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key), data2 + pos, pos, 10);
  check_sentinel_data(ctx, A_id);
  ...
  pos = 32;
  len = nanotube_map_write_masked(ctx, A_id, test_key, sizeof(test_key), data_splat,
  ...
  pos = 0;
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key), data2 + pos, pos,
                          sizeof(data2));
  check_sentinel_data(ctx, A_id);
  printf("Read %zu bytes with offset %zu:\n", len, pos);
  print_bytes(data2, 64); printf("\n");
  ...
  len = nanotube_map_read(ctx, illegal_id, test_key, sizeof(test_key), data2, 0, 64);
  check_sentinel_data(ctx, A_id);
  printf("Read %zu bytes from map %i\n", len, illegal_id);
  assert_eq(len, 0);

  /* No key */
  len = nanotube_map_read(ctx, A_id, NULL, sizeof(test_key), data2, 0, 64);
  check_sentinel_data(ctx, A_id);
  printf("Read %zu bytes from NULL key\n", len);
  assert_eq(len, 0);

  /* No data */
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key), NULL, 0, 64);
  check_sentinel_data(ctx, A_id);
  printf("Read %zu bytes to NULL data\n", len);
  assert_eq(len, 0);

  /* Missing key */
  uint8_t missing_key[KEY_WIDTH] = {1, 2, 3, 4, 5, 6, 7, 8};
  assert_eq(!memcmp(missing_key, test_key, sizeof(test_key)), 0);
  ...
  /* Too big key */
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key) + 3, data2, 0, 64);
  check_sentinel_data(ctx, A_id);
  ...
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key), huge_data, 0,
              sizeof(huge_data));
  check_sentinel_data(ctx, A_id);
  ...
  len = nanotube_map_read(ctx, A_id, test_key, sizeof(test_key), data2, pos,
              sizeof(data2));
  check_sentinel_data(ctx, A_id);
  ...
  /* Illegal writes */
  /* Wrong map */
  len = nanotube_map_write_masked(ctx, illegal_id, test_key, sizeof(test_key), data,
                                  all_enabled, 0, 64);
  check_sentinel_data(ctx, A_id);
  ...
  /* No key */
  len = nanotube_map_write_masked(ctx, A_id, NULL, sizeof(test_key), data,
                                  all_enabled, 0, 64);
  check_sentinel_data(ctx, A_id);
  ...
  /* No data */
  len = nanotube_map_write_masked(ctx, A_id, test_key, sizeof(test_key), NULL,
                                  all_enabled, 0, 64);
  check_sentinel_data(ctx, A_id);
  ...
  /* No enables */
  len = nanotube_map_write_masked(ctx, A_id, test_key, sizeof(test_key), data, NULL, 0,
                                  64);
  check_sentinel_data(ctx, A_id);
  ...
  /* Too big key */
  len = nanotube_map_write_masked(ctx, A_id, test_key, sizeof(test_key) + 3, data,
                                  all_enabled, 0, 64);
  check_sentinel_data(ctx, A_id);
  ...
  pos = 0;
  len = nanotube_map_write_masked(ctx, A_id, test_key, sizeof(test_key), huge_data,
  ...
  len = nanotube_map_update_masked(ctx, A_id, test_key, sizeof(test_key), data + pos,
  ...
  pos = 0;
  len = nanotube_map_insert_masked(ctx, A_id, test_key, sizeof(test_key), data + pos,
                                   all_enabled, pos, sizeof(data) / 2);
  check_sentinel_data(ctx, A_id);
  ...
  ++test_key[5];
  len = nanotube_map_remove(ctx, A_id, test_key, sizeof(test_key));
...
/*!
** Test for an error when inserting a non-existing entry.
**
*/
static void insert_error_test_1(nanotube_context_t *ctx,
                                nanotube_map_id_t A_id)
{
  size_t pos, len;
...
/*!
** Test for an error when removing an existing entry.
**
*/
static void remove_error_test_1(nanotube_context_t *ctx,
                                nanotube_map_id_t A_id)
{
  size_t len;
...
**
** Create the resources, run the requested set of tests and then
** destroys resources.
**/
void test_maps()
{
  ...
  map_A = nanotube_map_create(A_id, NANOTUBE_MAP_TYPE_ARRAY_LE,
                              KEY_WIDTH, 64);
  ...
  switch (test_case) {
  case 0:
    normal_map_tests(ctx, A_id);
    break;

  case 1:
    insert_error_test_1(ctx, A_id);
    break;

  case 2:
    remove_error_test_1(ctx, A_id);
    break;
