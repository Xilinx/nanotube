...
static const ModRefInfo nt_arg_info[][10] = {
  ...
  { N, _,             _,_,_,_,_,_,_,_},   /* llvm_lifetime_start */
  { N, _,             _,_,_,_,_,_,_,_},   /* llvm_lifetime_end */
  { W, N, N, N,           _,_,_,_,_,_},   /* llvm_memset */
  { W, R, N, N,           _,_,_,_,_,_},   /* llvm_memcpy */
  ...
  { R, N,             _,_,_,_,_,_,_,_},   /* context_add_map */
  { R, N, N,            _,_,_,_,_,_,_},   /* channel_create */
  { R, N, N,            _,_,_,_,_,_,_},   /* channel_set_attr */
  { R, N, N,            _,_,_,_,_,_,_},   /* channel_export */
  { R, N, W, N,           _,_,_,_,_,_},   /* channel_read */
  { RW, N, W, N,          _,_,_,_,_,_},   /* channel_try_read */
  { RW, N, R, N,          _,_,_,_,_,_},   /* channel_write */
  { R, N,             _,_,_,_,_,_,_,_},   /* channel_has_space */
  { R, R, _, R, N,          _,_,_,_,_},   /* thread_create */
  {               _,_,_,_,_,_,_,_,_,_},   /* thread_wait */
  { R, _, N, N,           _,_,_,_,_,_},   /* add_plain_packet_kernel */
  { R, W, N, N,           _,_,_,_,_,_},   /* packet_read */
  { W, R, N, N,           _,_,_,_,_,_},   /* packet_write */
  { W, R, R, N, N,          _,_,_,_,_},   /* packet_write_masked */
  {               _,_,_,_,_,_,_,_,_,_},   /* packet_edit */
  { R, N,             _,_,_,_,_,_,_,_},   /* packet_bounded_length */
  { R,             _, _,_,_,_,_,_,_,_},   /* packet_get_port */
  { W, N,             _,_,_,_,_,_,_,_},   /* packet_set_port */
  { R, N,             _,_,_,_,_,_,_,_},   /* packet_data */
  { R, N,             _,_,_,_,_,_,_,_},   /* packet_end */
  {RW, N, N,            _,_,_,_,_,_,_},   /* packet_resize */
  { W,RW, W, N, N,          _,_,_,_,_},   /* packet_resize_ingress */
  {RW, R, N,            _,_,_,_,_,_,_},   /* packet_resize_egress */
  {RW, N,             _,_,_,_,_,_,_,_},   /* packet_drop */
  {               _,_,_,_,_,_,_,_,_,_},   /* map_create */
  { R, N, N, R, N, R, W, R, N, N,    },   /* map_op */
  { R, N, N, R, N, R, R, N, N,      _},   /* map_op_send */
  { R, N, W, N,           _,_,_,_,_,_},   /* map_op_receive */
  {               _,_,_,_,_,_,_,_,_,_},   /* map_get_id */
  { R, N, R, N, N,          _,_,_,_,_},   /* map_lookup */
  { R, N, R, N, W, N, N,        _,_,_},   /* map_read */
  { R, N, R, N, R, N, N,        _,_,_},   /* map_write */
  { R, N, R, N, R, R, N, N,       _,_},   /* map_insert */
  { R, N, R, N, R, R, N, N,       _,_},   /* map_update */
  { R, N, R, N,           _,_,_,_,_,_},   /* map_remove */
  {               _,_,_,_,_,_,_,_,_,_},   /* get_time_ns */
  { N, N,             _,_,_,_,_,_,_,_},   /* debug_trace */
  { N, R, N,            _,_,_,_,_,_,_},   /* trace_buffer */
  { W,              _,_,_,_,_,_,_,_,_},   /* tap_packet_resize_ingress_state_init */
  { W,              _,_,_,_,_,_,_,_,_},   /* tap_packet_resize_egress_state_init */
  { N, N, N, N, N,          _,_,_,_,_},   /* tap_map_create */
  {RW, N, N, N, N,RW, N,RW, N,      _},   /* tap_map_add_client */
  {RW,              _,_,_,_,_,_,_,_,_},   /* tap_map_build */
};
const unsigned nt_arg_info_size = sizeof(nt_arg_info) / sizeof(nt_arg_info[0]);
#undef R
#undef W
#undef RW
#undef N
...
ModRefInfo get_nt_arg_info(Intrinsics::ID intr, unsigned arg_idx) {
  ...
  assert(high - low + 1 == nt_arg_info_size);
  assert(arg_idx < sizeof(nt_arg_info[0]) / sizeof(nt_arg_info[0][0]));

  return nt_arg_info[intr - low][arg_idx];
}
...
#define UK  FMRB_UnknownModRefBehavior
  ...
  static const FunctionModRefBehavior nt_fmrb[] = {
    UK,  /* none */
    DNA, /* llvm_bswap */
    UK,  /* llvm_dbg_declare */
    UK,  /* llvm_dbg_value */
...
#undef UK
  ...
  const unsigned low  = Intrinsics::none;
  const unsigned high = Intrinsics::end-1;
  const unsigned nt_fmrb_size = sizeof(nt_fmrb) / sizeof(nt_fmrb[0]);

  assert((intr >= low) && (intr <= high));
  assert(high - low + 1 == nt_fmrb_size);
  return nt_fmrb[intr - low];
}
};
